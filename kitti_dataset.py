...
# from visualize import point_visualize


class OdometryDataset():
    def __init__(self, root_path = '../', npoints=8192, is_training=True, random_seed = 3, sequences=range(7)):
        ...
        self.sequences = ['{:02d}'.format(i) for i in sequences]
        self.list = list(sequences)
        ...
        for seq in self.sequences:
            velo_path = os.path.join(self.datapath, seq, 'velodyne')
            frames = np.sort([vf[:-4] for vf in os.listdir(velo_path) if vf.endswith('.bin')])
            self.frames.extend(frames)


    def __getitem__(self, index):#
        index = index + self.len_list[self.list[0]]
        ...
        #     sample_idx1 = np.random.choice(indices_1, self.npoints, replace=False)
            # sample_idx1 = np.linspace(0, len(indices_1)-1, self.npoints, dtype=int)
            # sample_idx1 = indices_1[sample_idx1]
        ...
        #     sample_idx2 = np.random.choice(indices_2, self.npoints, replace=False)
            # sample_idx2 = np.linspace(0, len(indices_2) - 1, self.npoints, dtype=int)
            # sample_idx2 = indices_2[sample_idx2]
        # else:
        ...
        # sample_idx1 = np.sort(sample_idx1)
        # sample_idx2 = np.sort(sample_idx2)
        # pos1_ori = pos1
        ...
        # pos1 = pos1[indices_1, :]
        # pos2 = pos2[indices_2, :]
        # pos1_ori.astype('float32').tofile('./ori.bin')
        # pos1.astype('float32').tofile('./sam.bin')
        ...
        if self.is_training == True:  ###  augment
            ...
            add_T3 = np.ones((n, 1))
            pos2_trans = np.concatenate([pos2[:, :3], add_T3], axis=-1)
            pos2_trans = np.matmul(T_trans, pos2_trans.T)
            pos2 = np.concatenate([pos2_trans.T[:, :3],pos2[:, 3:4]], axis=-1)

            add_T = np.ones((n, 1))
            pos2_gt = np.concatenate([pos2[:, :3], add_T], axis=-1)
            pos2_gt = np.matmul(T_diff, pos2_gt.T)
            pos2_gt = np.concatenate([pos2_gt.T[:, :3], pos2[:, 3:4]], axis=-1)
