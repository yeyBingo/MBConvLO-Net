...
class kittiOdomEval():
    ...
    def plotPath_2D_3(self, seq, poses_gt, poses_result, plot_path_dir):
        ...
        start_point = [0, 0]
        style_pred = 'r-'# Line pred
        style_gt = 'k-'# Line GT
        style_O = 'ko' # Start Point
        style_1 = 'k^' # End Point GT
        style_2 = 'r^' # End Point pred
        ...
        plt.subplot(1, 3, 1)
        ax = plt.gca()
        if poses_gt: plt.plot(x_gt, z_gt, style_gt, label=plot_keys[0],linewidth=3)
        plt.plot(x_pred, z_pred, style_pred, label=plot_keys[1])
        plt.plot(start_point[0], start_point[1], style_O, label='Start Point')
        plt.plot(x_gt[-1], z_gt[-1], style_1, label='End Point')
        plt.plot(x_pred[-1], z_pred[-1], style_2)
        ...
        plt.plot(start_point[0], start_point[1], style_O, label='Start Point')
        plt.plot(x_gt[-1], y_gt[-1], style_1, label='End Point')
        plt.plot(x_pred[-1], y_pred[-1], style_2)
        plt.legend(loc="upper right", prop={'size': fontsize_})
        plt.xlabel('x (m)', fontsize=fontsize_)
        ...
        plt.plot(start_point[0], start_point[1], style_O, label='Start Point')
        plt.plot(y_gt[-1], z_gt[-1], style_1, label='End Point')
        plt.plot(y_pred[-1], z_pred[-1], style_2)
        plt.legend(loc="upper right", prop={'size': fontsize_})
    ...
    def plotPath_3D(self, seq, poses_gt, poses_result, plot_path_dir):
        ...
        # plt.plot(plane_point[-1:0], plane_point[-1:1], plane_point[-1:,2], style_1, label='End Point')
        xlim = ax.get_xlim3d()
        ylim = ax.get_ylim3d()
        zlim = ax.get_zlim3d()
        xmean = np.mean(xlim)
    ...
    def eval(self, toCameraCoord):
        ...
        for seq in self.eval_seqs:
            ...
            # print("seq" + str(seq).zfill(2) + " Average_t_error {0:.2f}".format(
            #     ave_t_err * 100) + " Average_r_error {0:.2f}".format(ave_r_err / np.pi * 180 * 100))

        return ave_t_err,ave_r_err

def kittieval(ii,args):

    # parser = argparse.ArgumentParser(description='KITTI Evaluation toolkit')


    # args = parser.parse_args()
    args.eva_seqs = str(ii).zfill(2) + "_pred"
    pose_eval = kittiOdomEval(args)
    ave_t_err, ave_r_err = pose_eval.eval(toCameraCoord=args.toCameraCoord)  # set the value according to the predicted results
    return ave_t_err, ave_r_err
